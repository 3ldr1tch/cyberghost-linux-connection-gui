...
# ISO 3166-1 alpha-2 catalog. This is not a CyberGhost availability list.
ISO_COUNTRIES = [
...
class Directory:
    ...
    def call(self, *args: str) -> str:
        cp = run(["sudo", "env", f"HOME={self.runtime_home}", str(self.binary), *args], 90)
        if cp.returncode:
...
def main() -> int:
    ...
    try:
        countries = [(f"{name} ({code})", (code.lower(), name))
                     for code, name in sorted(ISO_COUNTRIES, key=lambda x: x[1])]
